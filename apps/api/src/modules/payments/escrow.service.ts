...
export class EscrowService {
  ...
  async depositForUser(
    ...
    try {
      ...
      if (error instanceof DuplicateEscrowIdempotencyKeyError) {
        const existing =
          await this.escrowRepository.findByIdempotencyKey(idempotencyKey);
        if (existing) {
          return existing;
        }
      }
    ...
    try {
      ...
      await this.escrowRepository.updateStatus(escrow.id, {
        status: 'FAILED',
        failureCode:
          error instanceof SorobanInvocationError
  ...
  async releaseForUser(
    ...
    if (escrow.status !== 'LOCKED' || !escrow.onChainEscrowId) {
      throw new EscrowFailedError(
        `Escrow ${escrowId} is not in a releasable state`,
      );
    }

    const account = await this.stellarAccountRepository.findById(
      escrow.payerStellarAccountId,
    );
    if (!account) {
  ...
  async refundForUser(userId: string, escrowId: string): Promise<EscrowRecord> {
    ...
    if (escrow.status !== 'LOCKED' || !escrow.onChainEscrowId) {
      throw new EscrowFailedError(
        `Escrow ${escrowId} is not in a refundable state`,
      );
    }

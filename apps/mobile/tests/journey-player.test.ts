/**
 * Mobile Journey Player Tests
 *
 * Verifies the mobile-optimized JourneyPlayerMachine aligns with web behavior.
 * These tests mirror the web contract tests to ensure cross-platform consistency.
 */
...
test('reset() returns player to IDLE state', () => {
  const player = makePlayer(2, 100);
  player.start();
  player.tick(50);
  player.reset();
  const snap = player.getSnapshot();
  assert.equal(snap.playerState, PlayerState.IDLE);
  assert.equal(snap.tracks[0].state, TrackState.UNLOCKED);
  assert.equal(snap.tracks[1].state, TrackState.LOCKED);
  assert.equal(snap.currentTrackIndex, 0);
  assert.equal(snap.impressions.length, 0);
});
...
test('progress calculates correctly', () => {
  ...
  player.tick(50);
  assert.equal(player.progress, 50);
  player.tick(50);
  assert.equal(player.progress, 100);
});

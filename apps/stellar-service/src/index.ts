...
import {
  ...
  type StellarHealthResponse,
  type StellarServiceHandshake,
  type StellarAuthChallengeResponse,
  type StellarAuthVerifyResponse,
  type AuthAuditEntry,
...
function stellarRateLimit(maxReq: number, windowMs: number) {
  return function (req: Request, res: Response, next: NextFunction): void {
    ...
    res.setHeader(
      ...
      String(Math.max(0, maxReq - entry.count)),
    );
...
function logStellarAudit(
  event: AuthAuditEntry["event"],
  req: Request,
...
export function createStellarServiceApp(): Application {
  const app = express();
  ...
  app.post(
    ...
    (req: Request, res: Response) => {
      ...
      logStellarAudit("stellar_challenge", req, {
        keyPrefix: stellarPublicKey.slice(0, 8),
      });
  ...
  app.post(
    ...
    (req: Request, res: Response) => {
      ...
      logStellarAudit("stellar_verify", req, {
        keyPrefix: stellarPublicKey.slice(0, 8),
      });

      if (!isSupportedStellarSessionToken(sessionToken)) {
        logStellarAudit("login_failure", req, {
...
export function startStellarServiceServer() {
  const app = createStellarServiceApp();
  return app.listen(env.STELLAR_SERVICE_PORT, () => {
    console.log(
